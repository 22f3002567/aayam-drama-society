...
import { Play } from "@/types/schema";
import { getRasaForPlay } from "@/lib/rasa";
import { CardSize } from "@/lib/layout";

// Helper for YouTube Thumbnails
const getPosterSrc = (play: Play) => {
  if (play.poster_url) return play.poster_url;
  if (play.youtube_id)
    return `https://img.youtube.com/vi/${play.youtube_id}/maxresdefault.jpg`;
  return null;
};
...
export default function PlayCard({ play, size = "portrait" }: PlayCardProps) {
  ...
  // Detect mobile safely (no hydration mismatch)
  useEffect(() => {
    setIsMobile(window.matchMedia("(hover: none)").matches);
  }, []);

  const canPlayVideo =
    size === "titan" && Boolean(play.youtube_id) && !isMobile;
  ...
  return (
    ...
    >
      <Link
        ...
        onClick={() => {
          // Safety: cancel autoplay on click
          setAllowVideo(false);
          if (hoverTimerRef.current) {
            clearTimeout(hoverTimerRef.current);
          }
      ...
      >
        {/* GLOW */}
        <div
          className="absolute -inset-1 rounded-lg blur-xl transition-opacity duration-500"
          style={{
        ...
        {/* MASK / CARD */}
        <div className="relative w-full h-full overflow-hidden rounded-sm bg-neutral-900 border border-white/10 ring-1 ring-white/5 transition-colors duration-300 group-hover:border-white/30" />
        ...
        <div className="absolute inset-0 z-0">
          {initialSrc && !imgError ? (
            ...
            <div className="w-full h-full flex items-center justify-center relative">
              <div
                ...
                style={{ backgroundColor: rasa.color }}
              />
              <div className="absolute inset-0 bg-gradient-to-t from-black via-transparent to-transparent" />
              <span
                className="text-9xl font-serif font-bold opacity-10 select-none"
          ...
          {/* VIDEO PREVIEW (Desktop + Titan + Intent only) */}
          {canPlayVideo && allowVideo && (
            <div className="absolute inset-0 z-10">
              <iframe
                title={`Preview of ${play.title}`}
        ...
        <div className="absolute inset-0 z-30 flex flex-col justify-end p-6 md:p-8">
          ...
          <h3
            className={`font-serif text-white leading-tight tracking-tight mb-2 ${
              size === "titan" ? "text-5xl" : "text-2xl"
            } ${play.title.length > 20 && size !== "titan" ? "text-xl" : ""}`}
            style={{ textWrap: "balance" }}
          >
          ...
          <div
            className={`overflow-hidden transition-all duration-500 ${
              isHovered ? "max-h-[200px] opacity-100 mt-4" : "max-h-0 opacity-0"
            }`}
          >
            <p className="text-neutral-400 font-serif italic text-sm leading-relaxed line-clamp-3">
              {play.description || "A story waiting to be told."}
