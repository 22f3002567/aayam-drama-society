...
// // import { useRouter, useSearchParams } from "next/navigation"; // URL CONTROL
// // import { Play, PlayCategory } from "@/types/schema";
// // import PlayCard from "@/components/originals/PlayCard";

// // import ArchiveHeader from "@/components/originals/ArchiveHeader"; // NEW
...
// //   // Sync state if URL changes externally (back button)
// //   useEffect(() => {
...
// //         // We rely on the Server Component to have passed the correct initialPlays, 
// //         // but for client-side navigation, we might need to re-fetch if we didn't do a full refresh.
// //         // For simplicity in this Next.js App Router model, we will rely on the `handleCategoryChange` 
// //         // pushing a new route which triggers a server re-render effectively or we handle it here.
// //         // LET'S KEEP IT SIMPLE: Client-side fetch on change.
// //     }
// //   }, [searchParams]);
...
// //     // 1. Update URL (without full reload, just shallow push for history)
// //     // Actually, in App Router, replace is better for filters unless we want history stack
// //     router.push(`/originals?category=${cat}`, { scroll: false });
    
// //     setActiveCategory(cat);
// //     setPage(1);
// //     setPlays([]); // Clear for visual refresh
...
// //       <motion.div 
// //         layout // THE MAGIC PROP: Animates layout changes
// //         className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-12 gap-y-24 min-h-[50vh]"
// //       >
// //           <AnimatePresence mode="popLayout">
// //             {plays.map((play, index) => {
// //                 // RESPONSIVE STAGGER: Only apply translate-y on lg screens
