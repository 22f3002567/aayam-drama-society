...
// // import { Calendar, MapPin, MoreVertical, Eye, EyeOff } from "lucide-react";

// // export default function EventList({ events }: { events: any[] }) {
// //   if (!events?.length) return <div className="text-white/20 font-mono text-sm">No events recorded in the timeline.</div>;

// //   return (
// //     <div className="space-y-2">
// //       {events.map((event) => (
// //         <div key={event.id} className="group flex items-center justify-between p-4 bg-[#0a0a0a] border border-white/5 rounded-lg hover:border-white/20 transition-all">
          
// //           {/* LEFT: DATE & TITLE */}
// //           <div className="flex items-center gap-6">
// //              {/* The Date Block */}
// //              <div className="flex flex-col items-center justify-center w-12 h-12 bg-white/5 rounded border border-white/5 group-hover:border-gold-500/50 transition-colors">
// //                 <span className="text-[10px] font-mono uppercase text-white/40">{new Date(event.date).toLocaleString('default', { month: 'short' })}</span>
// //                 <span className="text-xl font-serif text-white">{new Date(event.date).getDate()}</span>
// //              </div>

// //              <div>
// //                 <h4 className="text-white font-medium group-hover:text-gold-500 transition-colors">{event.title}</h4>
...
// //                     </span>
// //                     <span className={`text-[10px] font-mono uppercase px-1.5 py-0.5 rounded ${event.type === 'show' ? 'bg-red-900/30 text-red-500' : 'bg-blue-900/30 text-blue-500'}`}>
// //                         {event.type}
// //                     </span>
// //                 </div>
// //              </div>
...
// //                   {new Date(event.date) < new Date() ? "Archived" : "Upcoming"}
// //               </span>
